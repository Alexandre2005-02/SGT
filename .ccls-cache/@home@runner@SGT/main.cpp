...
int main() {
    ...
    do {
        ...
        switch (opcao) {
            ...
            case 6: ordenarTarefas(); break;
            case 7: salvarDadosEmArquivo(); break;
...
void visualizarTarefas() {
    ...
    for (const auto& tarefa : tarefas) {
        cout << "ID: " << tarefa.id << ", Título: " << tarefa.titulo
             ...
             << ", Status: " << (tarefa.status == PENDENTE ? "Pendente" : tarefa.status == EM_ANDAMENTO ? "Em Andamento" : "Concluída")
             << ", Prioridade: " << (tarefa.prioridade == BAIXA ? "Baixa" : tarefa.prioridade == MEDIA ? "Média" : "Alta") << endl;
    }
}
    ...
    for (auto& tarefa : tarefas) {
        if (tarefa.id == codigo) {
            cout << "Novo título (atual: " << tarefa.titulo << "): ";
                ...
                auto it = remove_if(tarefas.begin(), tarefas.end(), [codigo](const Tarefa& tarefa) {
                return tarefa.id == codigo;
                });
                ...
                arquivo << tarefa.id << ";" << tarefa.titulo << ";" << tarefa.descricao << ";"
                    << tarefa.dataCriacao << ";" << tarefa.status << ";" << tarefa.prioridade << endl;
                }
                arquivo.close();
